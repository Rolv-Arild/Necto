...
def get_match(r, constant_reward=False, force_match_size=None):
...
def main():
    ...
    try:
        worker = make_worker(ip, name, password, limit_threads=True, send_gamestates=bool(strtobool(compress)), force_match_size=None)
        worker.run()
    finally:
